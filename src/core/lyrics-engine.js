...
window.LyricsEngine = {
    update: function(container, currentText, nextLines) {
        ...
        let htmlContent = `<div class="trans-line-current">${currentText}</div>`;
        ...
        if (nextLines && Array.isArray(nextLines) && nextLines.length > 0) {
            nextLines.forEach((line, index) => {
                ...
                htmlContent += `<div class="trans-line-next next-${index + 1}">${line}</div>`;
            });
        } else {
            // Keep layout stable if no next verses (End of Surah)
            htmlContent += `<div class="trans-line-next" style="opacity:0">&nbsp;</div>`;
        }
