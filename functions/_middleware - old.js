export async function onRequest(context) {
  ...
  if (lowerPath.startsWith('/media/')) {
    ...
    // ... inside the /media/ tunnel block ...

    // 3. Validate Token
    try {
        // Decode Base64 (handle URL-safe chars)
        const base64 = token.replace(/-/g, '+').replace(/_/g, '/');
        const decoded = atob(base64);
        const [timestampStr, salt] = decoded.split('|');

        // A. Verify Salt matches app.js
        if (salt !== "TUWA_SECURE_CLOCK_2026") {
            return new Response("Invalid Security Token", { status: 403 });
        }

        // B. Verify Timestamp (The "1 Second" Logic)
        const tokenTime = parseInt(timestampStr);
        const serverTime = Date.now();
        
        // Settings: How strict do you want to be?
        // ALLOWED_DRIFT: Allows user clock to be slightly wrong (ahead/behind)
        // LATENCY_BUFFER: Allows time for the request to travel
        // Total Window: ~30 seconds (Tight but Safe)
        const MAX_AGE = 30 * 1000; // 30 Seconds old
        const MAX_FUTURE = 10 * 1000; // 10 Seconds in the future (wrong user clock)

        // Check if token is too old
        if (serverTime - tokenTime > MAX_AGE) {
             return new Response("Link Expired (Refresh Page)", { status: 410 });
        }

        // Check if token is from the future (User clock is wrong)
        if (tokenTime - serverTime > MAX_FUTURE) {
             return new Response("Clock Skew Detected", { status: 403 });
        }
