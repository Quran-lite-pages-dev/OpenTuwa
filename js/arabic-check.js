...
window.addEventListener('load', function() {
    ...
    window.launchPlayer = function(chapter, verse) {
        ...
        // Handle YES: Force change BACK to css/index.css
        btnYes.onclick = function() {
            modal.style.display = 'none';
            if (cssLink) {
                cssLink.href = 'css/index.css'; // RESET to default
            }
            originalLaunchPlayer(chapter, verse);
        };

        // Handle NO: Change to css/index1.css
        btnNo.onclick = function() {
            modal.style.display = 'none';
            if (cssLink) {
                cssLink.href = 'css/index1.css'; // SWITCH to modified
            }
            originalLaunchPlayer(chapter, verse);
        };
    };
